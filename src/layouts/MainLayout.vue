...
const menuItems = [
  { name: 'หน้าหลัก', path: '/dashboard', icon: 'bi-grid-fill' },
...
const showAccountInfo = () => {
  ...
  Swal.fire({
    ...
    html: `
      <div class="text-left mt-5 space-y-4 bg-slate-50 p-6 rounded-[1.5rem] border border-slate-200/60 shadow-sm">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-full bg-gradient-to-br from-slate-800 to-slate-900 text-white flex items-center justify-center font-black text-2xl shadow-inner border border-slate-700 shrink-0">
                ${avatarChar.value}
            </div>
            <div class="overflow-hidden">
                <p class="font-black text-slate-800 text-xl leading-tight truncate">${authStore.currentUserName}</p>
                <p class="text-xs font-bold text-slate-400 tracking-widest uppercase mt-1">ID: ${authStore.userId}</p>
            </div>
        </div>
        <div class="h-px bg-slate-200/80 my-2"></div>
        <div class="space-y-3">
          <p class="text-sm text-slate-600 flex justify-between items-center"><b>ห้องปัจจุบัน:</b> <span class="font-black text-slate-800 truncate max-w-[150px]">${authStore.currentRoomName || '-'}</span></p>
          <p class="text-sm text-slate-600 flex justify-between items-center"><b>รหัสเข้าห้อง:</b> <span class="font-mono bg-white border border-slate-200 px-2.5 py-1 rounded-lg text-slate-700 font-bold text-xs shadow-sm">${authStore.currentRoomCode || '-'}</span></p>
          <p class="text-sm text-slate-600 flex justify-between items-center"><b>บทบาท (Role):</b> <span class="uppercase font-black text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-lg text-xs tracking-wider border border-indigo-100/50">${authStore.currentRole || '-'}</span></p>
        </div>
      </div>
    `,
    icon: 'info',
    ...
    customClass: {
      ...
      confirmButton: 'rounded-xl px-8 py-3 font-bold tracking-wide transition-all hover:shadow-lg active:scale-95'
    }
  });
};
            ...
            <transition name="fade-scale">
              <div v-if="activeDropdown === 'sidebarSettings'" class="absolute bottom-full left-0 mb-4 w-[240px] bg-white rounded-[1.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 py-3 z-50 origin-bottom-left cursor-default" @click.stop>
                ...
                <div class="px-2 space-y-1">
                  ...
                  <button @click="showAccountInfo" class="w-full text-left px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-colors flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-500"><i class="bi bi-info-circle"></i></div> ข้อมูลระบบ
                  </button>
      ...
      <header class="flex-shrink-0 bg-transparent z-30 pt-4 px-4 md:px-8">
        <div class="h-16 flex items-center justify-between bg-white/70 backdrop-blur-xl border border-slate-200/60 shadow-[0_4px_20px_rgba(0,0,0,0.02)] rounded-[1.5rem] px-5">
          ...
          <div class="flex items-center gap-3">
            <button @click="isSidebarOpen = true" class="md:hidden w-10 h-10 rounded-xl flex items-center justify-center text-slate-500 hover:bg-slate-100 transition-colors active:scale-95">
...
.fade-slide-leave-to { opacity: 0; transform: translateY(-15px) scale(0.99); }

.fade-scale-enter-active, .fade-scale-leave-active { transition: all 0.25s cubic-bezier(0.34, 1.56, 0.64, 1); }
